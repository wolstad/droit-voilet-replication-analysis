...
data.prob.wrong.indiv <- data.category %>%
  ...
  summarize(p.long.wrong = sum(responded == "long" && correct == FALSE) / n(), p.short.wrong = sum(responded == "short" && correct == FALSE) / n())
...
data.prob.wrong <- data.prob.wrong.indiv %>%
  ...
  summarize(p.long.wrong.mean = mean(p.long.wrong), sd.long =sd(p.long.wrong), se.long=sd.long/sqrt(n()), p.short.wrong.mean = mean(p.short.wrong), sd.short =sd(p.short.wrong), se.short=sd.short/sqrt(n()))
...
```{r}
ezANOVA(data.prob.wrong.indiv, wid=subject_id, between = condition, within=emotion, dv= p.long.wrong)
```
